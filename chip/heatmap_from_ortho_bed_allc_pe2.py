...
# Usage: python3.4 heatmap_from_ortho_bed_allc_pe.py [-1|-2] [-a|-r|-g|-n] [-p=num_proc] [-b=num_bins[,num_bins_stream]] [-s=stream_size] [-t=percentile] [-f=ortholog_file] <gff_file> <fpkm_file> <bed_file> [bed_file | allc_file]*
...
def processInputs( gffFileStr, fpkmFileStr, bedFileStrAr, numProc, numBins, numBinsStream, streamSize, percentile, genesIncluded, orthoFile, speciesInt, fpkmInt ):
	...
	info = '#from_script:heatmap_from_ortho_bed_allc_pe.py; numBins:{:d}; numBinsStream:{:d}; streamSize:{:d}; ortho_file:{:s}; gff_file:{:s}; fpkm_file:{:s}; chip_bed_files:{:s}'.format( numBins, numBinsStream, streamSize, os.path.basename(orthoFile), os.path.basename(gffFileStr) , os.path.basename(fpkmFileStr), ','.join(chipBedStr) )
	
	if genesIncluded == 'all':
		subsetAr = None
	else:
		print( 'Reading A. thaliana - E. salsugineum ortholog file...' )
		subsetDict = readGBMPairs( genesIncluded, orthoFile, speciesInt, fpkmInt )
		# subsetDict is dictionary with fpkm gene as key and report gene as value
	print( 'Reading FPKM file...' )
	print( list(subsetDict.keys())[0:3] )
	fpkmAr, fpkmValAr =  getFPKM( fpkmFileStr, subsetDict )
	print( fpkmValAr[0] )
...
def getFPKM( fpkmFileStr, subsetDict ):
	'''
	...
	for line in fpkmFile:
		...
		if i != -1:
			lineAr[0] = lineAr[0][:i]
		if subsetAr == None or lineAr[0] in subsetAr:
			#fpkmAr += [lineAr[0]]
...
def processFile( fileStr, outFileStr, fpkmAr, gffDict, numBins, numBinsStream, streamSize, percentile, chrmFormat, fpkmValAr, info ):
...
def processBed( bedDict, fpkmAr, geneDict, numBins, numBinsStream, streamSize ):
	...
	for gene in fpkmAr:
		...
		if info == None:
			...
			outAr = [-1] * (numBins + 2*numBinsStream)
...
def writeOutput( outFileStr, outMatrix, thresh, fpkmAr, fpkmValAr, info ):
	...
	headerAr = [ 'geneName', 'fpkm', 'bin', 'value' ]
	outFile.write( info + '\n' + '\t'.join( headerAr ) + '\n' )
	...
	for i in range( len(outMatrix) ):
		...
		for j in range(len(vals)):
			if vals[j] > thresh:
				vals[j] = thresh
			outStr = '{:s}\t{:s}\t{:d}\t{:.4f}\n'.format( fpkmAr[i], fpkmValAr[i], j, vals[j] )
...
if __name__ == "__main__":
	if len(sys.argv) < 4 :
		print ("Usage: python3.4 heatmap_from_ortho_bed_allc_pe2.py [-1[,2]|-2[,1]] [-a|-r|-g|-n] [-p=num_proc] [-b=num_bins[,num_bins_stream]] [-s=stream_size] [-t=percentile] [-f=ortholog_file] <gff_file> <fpkm_file> <bed_file> [bed_file | allc_file]*\n-1\tuse genes (and fpkm values) listed first in ortholog file\n-1,2\tuse genes listed first in ortholog file but fpkm values are for genes listed second\n-2\tuse genes listed second in ortholog file\n-2,1\tuse genes listed second in ortholog file but fpkm values are for genes listed first-a\tinclude all genes\n-r\tinclude only orthologs\n-g\tinclude only gBM orthologs\n-n\tinclude only non-gBM orthologs\n-f=ortholog_file\tuse this ortholog file instead\np=num_proc\tnumber of processors to use [default 2]\n-b=N,W\tuse N bins for gene body and W bins for upstream/downstream\n-s\tnumber of base pairs to include upstream and downstream\n-t=D\tpercentile to correct largest values by")
	else:
