...
def processSpecies( gffFileStr, intergenicBedStr, allcFileStr, chipBedStr, fastaFileStr, outFileStr, unmethAr, methAr, speciesID, regionWidth, numCGs, numFeatures, numBins, numBinsStream, streamSize, percentile, fpkmMethAr, fpkmUnmethAr, info ):
	...
	nameMatrix, outMatrix, interMatrix, numList, metaAr = processBedRegions( chipBedStr, lowestIntergenic, highestIntergenic, unmethAr, methAr, gffDict, numBins, numBinsStream, streamSize )
	print( interMatrix[0:10] )
	# threshold
	...
	writeOutput( outFileStr, nameMatrix, outMatrix, interMatrix, thresh, numBins, numBinsStream, info, fpkmMethAr, fpkmUnmethAr )
	return metaAr
...
def processBedRegions( bedFileStr, interUnmethAr, interMethAr, geneUnmethAr, geneMethAr, gffDict, numBins, numBinsStream, streamSize ):
	...
	numList = []
	interMatrix = []
	metaAr = [0]*(4*numBins + 4*numBinsStream)
	
	# loop through all arrays together
	for i in range( len( interMethAr ) ):
		...
		# put together
		nameTmpAr = [ iuName, imName, guName, gmName ]
		numTmpAr = iuBedAr + imBedAr + guBedAr + gmBedAr
		interTmpAr = [ ('NA' if iuRegion[4] >= 0.05 else '0' ), ('NA' if iuRegion[5] >= 0.05 else '0' ), ('NA' if imRegion[4] >= 0.05 else '0' ), ('NA' if imRegion[5] >= 0.05 else '0' ) ]
		# adjust by readCounts
		numTmpAr = [ x / readCounts * 1000000 for x in numTmpAr ]
		nameMatrix.append( nameTmpAr )
		outMatrix.append( numTmpAr )
		interMatrix.append( interTmpAr )
		numList += numTmpAr
		metaAr = addToArray( metaAr, numTmpAr )
	return nameMatrix, outMatrix, interMatrix, numList, metaAr
...
def writeOutput(outFileStr, nameMatrix, outMatrix, interMatrix, thresh, numBins, numBinsStream, info, fpkmMethAr, fpkmUnmethAr):
	outFile = open( outFileStr, 'w' )
	...
	headerAr = ['interUnmethRegion', 'interMethRegion', 'geneUnmethName', 'geneMethName', 'geneUnmethFPKM','geneMethFPKM', 'rowNum']
	headerAr += [ '{:d}_interUnmeth'.format( x ) for x in range( numBins ) ]
	headerAr += [ '{:d}_CHGmethylated'.format( numBins ), '{:d}_CHHmethylated'.format( numBins+1 ) ]
	headerAr += [ '{:d}_interMeth'.format( x ) for x in range( numBins+2, 2*numBins+2 ) ]
	headerAr += [ '{:d}_CHGmethylated'.format( 2*numBins+2 ), '{:d}_CHHmethylated'.format( 2*numBins+3 ) ]
	headerAr += [ '{:d}_geneUnmeth'.format( x ) for x in range( 2*numBins+4, 3*numBins+2*numBinsStream+4 ) ]
	headerAr += [ '{:d}_geneMeth'.format( x ) for x in range( 3*numBins+2*numBinsStream+4, 4*numBins+4*numBinsStream+4) ]
	outFile.write( info + '\n' )
	...
	for i in range( len( outMatrix ) ):
		names = nameMatrix[i]
		#print( names )
		rNum = len(outMatrix) - i
		vals = outMatrix[i]
		for j in range(len(vals)):
			if vals[j] > thresh:
				vals[j] = thresh
		valStr = [ '{:.5f}'.format( x ) for x in vals ]
		valStr2 = valStr[:numBins] + interMatrix[i][:2] + valStr[numBins:2*numBins] + interMatrix[i][2:] + valStr[2*numBins:]
		outFile.write( '{:s}\t{:.2f}\t{:2f}\t{:d}\t{:s}\n'.format( '\t'.join(names),  fpkmUnmethAr[i],fpkmMethAr[i],rNum,'\t'.join(valStr2) ) )
