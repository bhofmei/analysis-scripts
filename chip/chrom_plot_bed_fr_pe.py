...
# Usage: chrom_plot_bed_fr_pe.py [-o=out_id] [-p=num_proc] [-b=num_bins | -s=bin_size] [-c=chrm_list] [-l=labels] [-t=percentile] <fasta_index> <bed_file> [bed_file]*
...
def processInputs( bedFileStrAr, fastaIndexStr, labels, outID, chrmList, numProc, numBins, binSize, percentile ):
	...
	info = "#from_script:chrom_plot_bed_fr_pe.py; "
	if binSize == -1:
	...
	info += "unit:{:s} per ".format( 'total bp' )
	if binSize == -1:
		info += '10kb'
...
def processBEDFile( bedFileStr, chrmDict, binSize, numBins, percentile ):
	...
	outDict = processBEDFileHelper( bedDict, bpCounts, chrmDict, binSize, numBins )
	# percentile
...
def printHelp():
	outStr = "Usage: python3 chrom_plot_bed_fr_pe.py [-o=out_id] [-p=num_proc] [-b=num_bins | -s=bin_size] [-c=chrm_list] [-l=labels] [-t=percentile] <fasta_index> <bed_file> [bed_file]*\n\n"
	outStr += 'Required:\n'
	...
	outStr += '-l=labels\tcomma-separated list of labels for BED files [default: bed file name]\n'
