...
# Usage: python3.4 metaplot_from_bed_allc_fr_pe.py [-1|-2] [-a|-r|-g|-n] [-p=num_proc] [-b=num_bins[,num_bins_stream]] [-s=stream_size] [-o=output_identifer] <gff_file> <bed_file> [bed_file | allc_file]*
...
def processInputs( gffFileStr, bedFileStrAr, numProc, numBins, numBinsStream, streamSize, outID, genesIncluded, orthoFile, speciesInt ):
	...
	if outID == '' and genesIncluded == 'all':
		...
		outFileStr = 'meta_{:d}_{:d}_{:s}.tsv'.format(numBins,streamSize,genesIncluded)
	...
	sampleFiles = [ os.path.basename(x) for x in bedFileStrAr ]
	info = '#from_script:metaplot_from_ortho_bed_allc_fr_pe.py; numBins: {:d}; numBinsStream:{:d}; Upstream/downstream size:{:d}; genes_included:{:s}; ortho_file:{:s}; gff_file:{:s}; sample_files:{:s}'.format( numBins, numBinsStream, streamSize, genesIncluded, os.path.basename(orthoFile), os.path.basename(gffFileStr), ','.join(sampleFiles) )
	...
	writeOutput( outFileStr, varMatrix, sampleNamesAr, info )
	print( 'Done.' )
...
def processFile( gffAr, fileStr, numBins, numBinsStream, streamSize, chrmF ):
	isBed = checkFile( fileStr )
	if isBed:
		bedFile = FileBED_FR( fileStr )
		print( 'Reading BED file {:s}'.format( str(bedFile) ) )
		bedDict, bpCount = bedFile.getBedDict()
		print( 'Processing {:s}'.format( str(bedFile) ) )
		outMat = processBed( gffAr, bedDict, bpCount, numBins, numBinsStream, streamSize )
		del(bedFile, bedDict)
	else:
		allcFile = FileAllC_full( fileStr )
		print( 'Reading allC file {:s}'.format( str(allcFile) ) )
		allCDictA = allcFile.getAllCDict( chrmFormat=chrmF )
		allCDict = allCDictA['CG']
		print( 'Processing {:s}'.format( str(allcFile) ) )
		outMat = processAllC( gffAr, allCDict, numBins, numBinsStream, streamSize )
		del(allcFile, allCDictA, allCDict)
	return outMat
...
def processBed( geneAr, bedDict, bpCounts, numBins, numBinsStream, streamSize ):
...
def writeOutput( outFileStr, varMatrix, sampleNamesAr, info ):
	'''
		writes output to outFileStr
		output for all input files is written to one file
	'''
	headerAr = ['sample','bin','value']
