...
# Usage: python3 features_covered_reads.py [-d] [-w=min_fraction_covered] [-o=out_id] [-s=promoter_size] [-f=fasta_index] [-r=repeat_gff] <gene_gff <bed_file>

LOWFRAC=0.05
PROMOTSIZE=1000

def processInputs( gffFileStr, bedFileStr, repeatGffFile, fastaIndexFile, outID, lowerFractionCovered, promoterSize, isDebug ):
	
	info = '#from_script:features_covered_reads.py; gff_file:{:s}; bed_file:{:s}'.format( os.path.basename( gffFileStr ), os.path.basename( bedFileStr ) )
	print( 'Reading GFF' )
	...
	if isDebug:
		debugFileStr = outID + '_reads.tsv'
	else:
		debugFileStr = None
	
	# read BED file
	print( 'Reading and Analyzing BED File' )
	outDict = readBedFile( bedFileStr, featDict, lowerFractionCovered, debugFileStr )
...
def readBedFile( bedFileStr, featDict, lowerFractionCovered, readsDebugStr ):
	outCountDict = { 'g':0,'p':0,'t':0,'i':0,'a':0}
	chrms = list( featDict.keys() )
	
	if readsDebugStr != None:
		readsOutFile = open( readsDebugStr, 'w' )
		headerAr = ['chrm','start','end','name','assignment','g','i','p','t']
		readsOutFile.write( '#{:s}\n'.format( '\t'.join( headerAr ) ) )
	...
	for line in bedFile:
		...
		for i in range(start, end+1):
			try:
				...
				print( 'ERROR: {:d} out of range for chrm {:s}'.format( i, chrm ) )
		readType = defineReadType( readDict, (end-start+1), lowerFractionCovered)
		
		# debuging if necessary
		if readsDebugStr != None:
			tmpVals = [ str(readDict[x]) for x in sorted(readDict.keys()) ]
			outStr = '{:s}\t{:d}\t{:d}\t{:s}\t{:s}\t{:s}\n'.format( chrm, start, end, lineAr[3], ','.join(readType), '\t'.join( tmpVals ) )
			readsOutFile.write( outStr )
		# update counter(2)
		for x in readType:
			outCountDict[x] += 1
		outCountDict['a'] += len( readType )
...
def defineReadType( inDict, readLen, lowFrac ):
	typesAr = ['g', 'p', 't', 'i']
	valsAr = [ float( inDict[typ] ) / readLen for typ in typesAr ]
	outAr = []
	for i in range(len(valsAr)):
		if valsAr[i] > lowFrac:
			outAr += typesAr[i]
	return outAr
...
def writeOutput( outDict, outFileStr, info ):
	...
	peakTypes = ['g','p','t','i']
	peakTypeFormat = [ 'genes', 'promoters', 'transposons/repeats', 'intergenic']
...
def parseInputs( argv ):
	...
	processInputs( gffFileStr, bedFileStr, repeatGffFile, fastaIndexFile, outID, lowerFractionCovered, promoterSize, isDebug )
