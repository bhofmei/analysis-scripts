...
# Usage: python compare_dmr_gens_pe.py [-o=outID] [-m=methTypes] [-p=numProc] [-v=minCov] <dmrFile> <allcPath>  <sample1> <sample2> [sampleN]*
...
def processInputs( dmrFileStr, allcPath, sampleNamesAr, outID, methType, numProc, minCov ):
	...
	info = '#from_script: compare_dmr_gens_pe.py; dmr_file: {:s}; samples: {:s}; methyl_type: {:s}; min_cov: {:d}'.format( os.path.basename( dmrFileStr ), ','.join( sampleNamesAr ), methType, minCov )
	...
	for i in range(len(sampleNamesAr)-1):
		...
		if mDict1 == None:
			mFile1 = FileAllC_full( os.path.normpath( '{:s}/allc_{:s}.tsv'.format( allcPath, sampleNamesAr[i] ) ) )
			mDict1 = mFile1.getAllCDict( mtypes = methType )
			del(mFile1)
		
		mFile2 = FileAllC_full( os.path.normpath( '{:s}/allc_{:s}.tsv'.format( allcPath, sampleNamesAr[i+1] ) ) )
		...
		results = [ pool.apply_async(processChrm, args=(mDict1[x], mDict2[x], dmrDict[x], comparisonName, minCov, x) ) for x in sorted(dmrDict.keys()) ]
...
def processChrm( mDict1, mDict2, dmrs, label, minCov, chrm ):
	...
	for i in range(len(dmrs)):
		...
		#print( dmrs[i] )
		# analyze region
		regionStats = processRegion( rStart, rEnd, mDict1, mDict2, minCov )
...
def processRegion( start, end, mDict1, mDict2, minCov ):
	...
	# get positions in common
	else:
		cSet1 = countCs( start, end, mDict1, minCov )
		cSet2 = countCs( start, end, mDict2, minCov )
		cSet = cSet1 & cSet2
		cCount = len(cSet)
		#print(cSet1, cSet2,cSet)
		mCount1, tCount1 = posCs( cSet, mDict1 )
		mCount2, tCount2 = posCs( cSet, mDict2 )
	# create output array
...
def countCs( start, end, inDict, minCov ):
	outAr = []
	for pos in range(start, end+1 ):
		tup = inDict.get( pos )
		if tup != None and tup[1] >= minCov:
			outAr += [ pos ]
	# end for
	return set(outAr)
...
def printHelp():
	print( 'Usage: python compare_dmr_gens_pe.py [-o=outID] [-m=methType] [-p=numProc] <dmrFile> <allcPath> <sample1> <sample2> [sampleN]*' )
