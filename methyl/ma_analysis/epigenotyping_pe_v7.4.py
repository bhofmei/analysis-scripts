...
# Usage: python epigenotyping_pe_v7.4 [-q] [-n-mpv] [-t-out] [-g=generation]  [-c=bin_comb_thresh] [-d=decoding_type] [-p=num_proc] [-o=out_id] [-m=mother_samples] [-f=father_samples] [-b=bin_size] [-t=cent_start,cent_end[,cent2_start,cent2_end]] <input_file>
...
def checkMidparents( df, nbins ):
	...
	# create copies of original probabilities
	if len(im[0]) == nbins or len(ip[0]) == nbins:
		df['father.uncorrected'] = df['father']
		df['MPV.uncorrected'] = df['MPV']
		df['mother.uncorrected'] = df['mother']
	if len(im[0]) == nbins:
		# all predicted to be mother, adjust father
		df['father'] = np.log( (np.exp(df['MPV']) + np.exp(df['father'])) / (1.0+np.exp(df['MPV'])) )
		df['MPV'] = np.log( np.exp(df['MPV']) / (1.0+np.exp(df['MPV'])) )
		df['mother'] = np.log( np.exp(df['mother']) / (1.0+np.exp(df['MPV'])) )
		#df['MPV'] = 0
	elif len(ip[0] ) == nbins:
		# all predicted to be father, adjust mother
		df['mother'] = np.log( (np.exp(df['MPV']) + np.exp(df['mother'])) / (1.0+np.exp(df['MPV'])) )
		df['MPV'] = np.log( np.exp(df['MPV']) / (1.0+np.exp(df['MPV'])) )
		df['father'] = np.log( np.exp(df['father']) / (1.0+np.exp(df['MPV'])) )
	return df
