...
from adaptivetransitions import *

# Usage: python epigenotyping_combin_iter-trans.py [-u] [-c=bin_comb_thresh] [-d=decoding_type] [-p=num_proc] [-o=out_id] [-m=mother_sample] [-f=father_sample] [-b=bin_size] [-n=max_iter] <input_file>
...
def processInputs( inFileStr, numProc, binSize, outID, parentLabelAr, decoding, isUniform, combineBins, maxIter ):
	...
	info = '#from_script: epigenotyping_combin_iter-trans.py; in_file:{:s}; bin_size:{:s}; decoding:{:s}; uni_class_prob:{:s}; combine_bins_threshold:{:d}; maximum_iterations:{:d}'.format( os.path.basename( inFileStr ), bth_util.binSizeToStr( binSize ), formatDecoding( decoding).lower().replace('and',','), str(isUniform).lower(), combineBins, maxIter )
	...
	if decoding != 'N':
		...
		if maxIter > 0:
			...
			at = AdaptiveTransitions( dfClass, transitionMatrix, ignoreAr, maxIter )
