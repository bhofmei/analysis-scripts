...
# Usage: python epigenotyping_pe_v8.1.py [-e] [-q] [-c=bin_comb_thresh] [-d=decoding_type] [-p=num_proc] [-o=out_id] [-m=mother_samples] [-f=father_samples] [-b=bin_size] [-t=cent_start,cent_end[,cent2_start,cent2_end]] <input_file>
...
def classify( df, parentLabelAr, classProbs ):
	...
	if classProbs == 'E':
		# epiRILs
		clf = linear_model.LogisticRegression(solver='lbfgs', class_weight={'mother':0.5,'MPV':0,'father':0.5}, max_iter=200)
		tmp = [ 0 if x.startswith('MPV') else 1 for x in trainLabels ]
		sampleWeight = np.array( tmp )
	elif classProbs == 'U':
		clf = linear_model.LogisticRegression(solver='lbfgs', class_weight={'mother':1,'MPV':2,'father':1})
	else:
...
def determineOutputFileName( inFileStr, outID, binSize, decoding, classProbs, combineBins ):
	...
	if outID == None:
		if '_wm_pos_' in inFileStr:
			outFileStr = outBaseName.replace( '_wm_pos_', '_epigenotype-v8.1_{:s}_'.format( bth_util.binSizeToStr(binSize) ) )
		else:
			outFileStr = 'out_epigenotype-v8.1_{:s}.tsv'.format( bth_util.binSizeToStr(binSize) )
	else:
		outFileStr = '{:s}_epigenotype-v8.1_{:s}.tsv'.format( outID, bth_util.binSizeToStr(binSize) )
	...
	elif decoding != 'N' and classProbs == 'U':
...
def printHelp():
	print( 'Usage:\npython epigenotyping_pe_v8.py [-e | -u] [-q] [-c=bin_thresh] [-d=decoding_type][-p=num_proc]\n[-o=out_id] [-m=mother_sample(s)] [-f=father_sample(s)] [-b=bin_size] <input_file>' )
	print( 'Requried:' )
	...
	print( '-u\t\tequal class weights [default is 1:2:1 mother,MPV,father]' )
