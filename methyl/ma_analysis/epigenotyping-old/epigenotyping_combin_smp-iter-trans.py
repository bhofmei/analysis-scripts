...
from transitionssmp import *

# Usage: python epigenotyping_combin_smp-iter-trans.py [-u] [-c=bin_comb_thresh] [-d=decoding_type] [-p=num_proc] [-o=out_id] [-m=mother_sample] [-f=father_sample] [-b=bin_size] [-n=max_iter] <input_file>
...
def runDecoding( dfg, numProc, transMat, decodeType, maxIter, transOutFileStr ):
	nt = ImproveTransitions, 
	mapfunc = partial( decodeSample, trans=transMat, d=decodeType, x=maxIter, f=transOutFileStr )
	with multiprocessing.Pool(processes=numProc) as p:
		res = p.map( mapfunc, [group for group in dfg] )
	return pd.concat(res)

def decodeSample( group, trans=None, d='V', x=10, f='out.txt' ):
	name, df = group
	# get improved transition matrix
	nt = ImproveTransitions( df, trans, x )
	newTransMat, niter = nt.getTransitions()
	print( '  Updated transition matrix for {:s}...{:s}'.format( name, ('did not converge' if niter == x else 'converged in '+str(niter)+ ' iterations' ) ) )
	with open( f, 'a' ) as ff:
		ff.write(name + '\n')
	newTransTable = pd.DataFrame(newTransMat, nt.labels, nt.labels)
	newTransTable.to_csv( f, sep='\t', mode='a' )
	#print('--',newTransMat.dtype)
	res = decode( df, newTransMat, d )
	res['sample'] = name
	return res
...
def determineTransFileName( inFileStr, outID, binSize, combineBins ):
	...
	if outID == None:
		if '_wm_pos_' in inFileStr:
			outFileStr = outBaseName.replace( '_wm_pos_', '_epigenotype-smp-trans_{:s}_'.format( bth_util.binSizeToStr(binSize) ) )
		else:
			outFileStr = 'out_epigenotype-smp-trans_{:s}.tsv'.format( bth_util.binSizeToStr(binSize) )
	else:
		outFileStr = '{:s}_epigenotype-smp-trans_{:s}.tsv'.format( outID, bth_util.binSizeToStr(binSize) )
...
def determineOutputFileName( inFileStr, outID, binSize, decoding, isUniform, combineBins ):
	...
	if outID == None:
		if '_wm_pos_' in inFileStr:
			outFileStr = outBaseName.replace( '_wm_pos_', '_epigenotype-smp-results_{:s}_'.format( bth_util.binSizeToStr(binSize) ) )
		else:
			outFileStr = 'out_epigenotype-smp-results_{:s}.tsv'.format( bth_util.binSizeToStr(binSize) )
	else:
		outFileStr = '{:s}_epigenotype-smp-results_{:s}.tsv'.format( outID, bth_util.binSizeToStr(binSize) )
...
def parseInputs( argv ):
	...
	for i in range( min(8, len(argv)-1) ):
		if argv[i].startswith( '-o=' ):
			...
			try:
				...
				print( 'WARNING: maximum iterations must be integer...using', MAXITER )
				maxIter = MAXITER
	...
	processInputs( inFileStr, numProc, binSize, outID, parentLabelAr, decoding, isUniform, combineBins, maxIter )
