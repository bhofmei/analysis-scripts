...
# Usage: python epigenotyping_pe_combbin.py [-u] [-c=bin_comb_thresh] [-d=decoding_type] [-p=num_proc] [-o=out_id] [-m=mother_sample] [-f=father_sample] [-b=bin_size] [-t=cent_start,cent_end] <input_file>
...
def decode( df, transMat, initProb, decodeType, cent ):
	# set up decoding
	if decodeType == 'A':
		alg = DecodeAll( df, transMat, initProb, cent )
	elif decodeType == 'F':
		alg = DecodeForwardBackward( df, transMat, initProb, cent )
	else:
		alg = DecodeViterbi( df, transMat, initProb, cent )
	# run decoding
	outdf = alg.run()
	return outdf
