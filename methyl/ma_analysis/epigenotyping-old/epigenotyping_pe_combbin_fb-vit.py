...
def processInputs( inFileStr, numProc, binSize, outID, parentLabelAr, decoding, isUniform, combineBins ):
	...
	info = '#from_script: epigenotyping_pe_combbin_fb-vit.py; in_file:{:s}; bin_size:{:s}; decoding:{:s}; uni_class_prob:{:s}; combine_bins_threshold:{:d}'.format( os.path.basename( inFileStr ), bth_util.binSizeToStr( binSize ), formatDecoding( decoding).lower().replace('and',','), str(isUniform).lower(), combineBins )
	...
	if decoding != 'N':
		...
		dfOutput = runDecoding( dfSampleGroup, numProc, transitionMatrix, tmpDecoding )
		...
		if decoding == 'B':
			...
			dfOutputN = runDecoding( dfSampleGroup, numProc, transitionMatrix, 'V' )
			dfOutput[['vit.score.mother', 'vit.score.father', 'vit.score.MPV', 'vit.prediction']] = dfOutputN[['vit.score.mother', 'vit.score.father', 'vit.score.MPV', 'vit.prediction']]
			#print( dfOutput.head() )
		# end decoding == B
