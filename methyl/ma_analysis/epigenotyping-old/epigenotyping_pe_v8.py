...
# Usage: python epigenotyping_pe_v8.py [-e] [-q] [-c=bin_comb_thresh] [-d=decoding_type] [-p=num_proc] [-o=out_id] [-m=mother_samples] [-f=father_samples] [-b=bin_size] [-t=cent_start,cent_end[,cent2_start,cent2_end]] <input_file>
...
def determineOutputFileName( inFileStr, outID, binSize, decoding, classProbs, combineBins ):
	...
	if outID == None:
		if '_wm_pos_' in inFileStr:
			outFileStr = outBaseName.replace( '_wm_pos_', '_epigenotype-v8_{:s}_'.format( bth_util.binSizeToStr(binSize) ) )
		else:
			outFileStr = 'out_epigenotype-v8_{:s}.tsv'.format( bth_util.binSizeToStr(binSize) )
	else:
		outFileStr = '{:s}_epigenotype-v8_{:s}.tsv'.format( outID, bth_util.binSizeToStr(binSize) )
...
def printHelp():
	print( 'Usage:\npython epigenotyping_pe_v8.py [-e] [-q] [-c=bin_thresh] [-d=decoding_type][-p=num_proc]\n[-o=out_id] [-m=mother_sample(s)] [-f=father_sample(s)] [-b=bin_size] <input_file>' )
	print( 'Requried:' )
