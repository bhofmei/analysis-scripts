...
class DecodePathBinWise:
	''' Base class for decoding types
	'''
	def __init__( self, df, transMatAr ):
		# n bins, m states
		self.labels = np.array( ['mother', 'MPV', 'father'] )
		self.data = df	# n x m
		self.transitionsAr = transMatAr # fraction probs not log, n-1 x m x m
...
class DecodeViterbiBinWise( DecodePathBinWise ):
	''' Viterbi decoding
	'''
	def run( self ):
	...
	def _computeScore( self, i, j, prob ):
		...
		for k in range(self.states):
			...
			scores[k] += self.log_transitions[k,j] # updated
		# end for k
		maxS = scores.max()
...
class DecodeForwardBackwardBinWise( DecodePathBinWise ):
