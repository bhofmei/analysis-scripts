...
# Usage: python epigenotype_by_logreg_pe.py [-u] [-i] [-s=decode_type] [-p=num_proc] [-o=out_id] [-b=bin_size] [-m=mother_sample] [-f=father_sample] <input_file>
...
def findOptimalPathMulti( gr, trans=np.array([]), u=False, d='V' ):
	name, df = gr
	res = findOptimalPath( df, trans, u, d )
	res['sample'] = name
	return res

def determineOutputFileName( inFileStr, outID, binSize, decoding, isUniform, isIndiv ):
	...
	if isIndiv:
		outFileStr = outFileStr.replace('.tsv', '-indiv.tsv' )
...
def parseInputs( argv ):
	...
	processInputs( inFileStr, numProc, binSize, outID, parentLabelAr, decoding, isUniform, isIndiv )
...
def printHelp():
	...
	print( '-i\t\ttransition probabilities computed individually for samples [default false]' )
