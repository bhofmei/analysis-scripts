...
# Usage: python epigenotyping_pe_v7 [-u | -e] [-q] [-c=bin_comb_thresh] [-d=decoding_type] [-p=num_proc] [-o=out_id] [-m=mother_samples] [-f=father_samples] [-b=bin_size] [-t=cent_start,cent_end[,cent2_start,cent2_end]] <input_file>
...
def classifyBin( group, pla=None, u='D' ):
	'''
...
def classify( df, parentLabelAr, classProbs ):
	...
	if classProbs == 'U':
		...
		# epiRILs
		clf = linear_model.LogisticRegression(class_weight={'mother':10,'MPV':0,'father':10})
...
def printHelp():
	print( 'Usage:\npython epigenotyping_pe_v7.py [-u | -e] [-q] [-c=bin_thresh] [-d=decoding_type][-p=num_proc]\n[-o=out_id] [-m=mother_sample(s)] [-f=father_sample(s)] [-b=bin_size] <input_file>' )
	print( 'Requried:' )
