...
from decodingpath4 import *
from transitions import Transitions

# Usage: python epigenotyping_pe_v9.py [-e] [-s] [-q] [-c=bin_comb_thresh] [-d=decoding_type] [-p=num_proc] [-o=out_id] [-m=mother_samples] [-f=father_samples] [-b=bin_size] [-t=cent_start,cent_end[,cent2_start,cent2_end]] <input_file>
...
def processInputs( inFileStr, numProc, binSize, outID, parentLabelAr, decoding, classProbs, combineBins, cent, scaleTransitions, isPrint ):
	
	info = '#from_script: epigenotyping_pe_v9.py; in_file:{:s}; bin_size:{:s}; decoding:{:s}; class_prob:{:s}; combine_bins_threshold:{:d}; centromere_{:s}; scale_transitions:{:s}'.format( os.path.basename( inFileStr ), bth_util.binSizeToStr( binSize ), formatDecoding( decoding).lower().replace('and',','), formatClassProbs(classProbs).lower(), combineBins, ('None' if cent == None else '{:s}-{:s}'.format( bth_util.binSizeToStr( cent[0] ), bth_util.binSizeToStr( cent[1] ) ) ), str(scaleTransitions) )
	if isPrint:
	...
	if decoding != 'N':
		...
		if scaleTransitions:
			scaleFactor = float( nsamples - len( tIgnoreAr ) - 1 ) / float( nsamples - len( tIgnoreAr ) )
		else:
			scaleFactor = 1
		...
		dfOutput = runDecoding( dfSampleGroup, numProc, transitionMatrix, tmpDecoding, centBins, scaleFactor )
		...
		if decoding == 'B':
			...
			dfOutputN = runDecoding( dfSampleGroup, numProc, transitionMatrix, 'V', centBins, scaleFactor )
	...
	# write output
	outFileStr = determineOutputFileName( inFileStr, outID, binSize, decoding, classProbs, scaleTransitions, combineBins )
...
def formatClassProbs( classProbs ):
	if classProbs == 'D':
		return 'Default'
	elif classProbs == 'E':
		return 'epiRIL'
	else:
		print( 'ERROR: Classification probability undefined' )
		exit()
...
def classify( df, parentLabelAr, classProbs ):
	...
	if classProbs == 'E':
		# epiRILs
		clf = linear_model.LogisticRegression(solver='lbfgs', class_weight={'mother':0.5,'MPV':0,'father':0.5})
		tmp = [ 0 if x.startswith('MPV') else 1 for x in trainLabels ]
		sampleWeight = np.array( tmp )
	else:
		clf = linear_model.LogisticRegression(solver='lbfgs')
	
	# fit training data
	clf.fit( train.values, trainClasses, sample_weight = sampleWeight )
...
def runDecoding( dfg, numProc, transMat, decodeType, centro, scaleFactor ):
	mapfunc = partial( decodeSample, trans=transMat, d=decodeType, cent=centro, sc=scaleFactor )
	with multiprocessing.Pool(processes=numProc) as p:
		res = p.map( mapfunc, [group for group in dfg] )
	return pd.concat(res)

def decodeSample( group, trans=None, d='V', cent=[], sc=1 ):
	name, df = group
	#print( name )
	res = decode( df, trans, d, cent, sc )
	res['sample'] = name
	return res

def decode( df, transMat, decodeType, cent, sc ):
	# set up decoding
	if decodeType == 'A':
		alg = DecodeAll( df, transMat, cent, sc )
	elif decodeType == 'F':
		alg = DecodeForwardBackward( df, transMat, cent, sc )
	else:
		alg = DecodeViterbi( df, transMat, cent, sc )
	# run decoding
	outdf = alg.run()
	return outdf
...
def determineOutputFileName( inFileStr, outID, binSize, decoding, classProbs, scaleTransitions, combineBins ):
	outBaseName = os.path.basename( inFileStr )
	if outID == None:
		if '_wm_pos_' in inFileStr:
			outFileStr = outBaseName.replace( '_wm_pos_', '_epigenotype-v9_{:s}_'.format( bth_util.binSizeToStr(binSize) ) )
		else:
			outFileStr = 'out_epigenotype-v9_{:s}.tsv'.format( bth_util.binSizeToStr(binSize) )
	else:
		outFileStr = '{:s}_epigenotype-v9_{:s}.tsv'.format( outID, bth_util.binSizeToStr(binSize) )
	...
	if scaleTransitions:
		outFileStr = outFileStr.replace('.tsv', '_scaled.tsv'.format( combineBins ) )
...
def parseInputs( argv ):
	...
	processInputs( inFileStr, numProc, binSize, outID, parentLabelAr, decoding, classProbs, combineBins, centromere, scaleTransitions, isPrint )

def printHelp():
	print( 'Usage:\npython epigenotyping_pe_v8.py [-e] [-s] [-q] [-c=bin_thresh] [-d=decoding_type][-p=num_proc]\n[-o=out_id] [-m=mother_sample(s)] [-f=father_sample(s)] [-b=bin_size] <input_file>' )
	print( 'Requried:' )
	...
	print( '-s\t\tscale transition probs by number of samples; useful for small sample size' )
