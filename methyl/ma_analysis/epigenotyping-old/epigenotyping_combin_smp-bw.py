...
from baumwelchtransitions import *

# Usage: python epigenotyping_combin_smp-bw.py [-u] [-c=bin_comb_thresh] [-d=decoding_type] [-p=num_proc] [-o=out_id] [-m=mother_sample] [-f=father_sample] [-b=bin_size] [-n=max_iter] <input_file>
...
def processInputs( inFileStr, numProc, binSize, outID, parentLabelAr, decoding, isUniform, combineBins, maxIter ):
	...
	info = '#from_script: epigenotyping_combin_smp-bw.py; in_file:{:s}; bin_size:{:s}; decoding:{:s}; uni_class_prob:{:s}; combine_bins_threshold:{:d}; maximum_iterations:{:d}'.format( os.path.basename( inFileStr ), bth_util.binSizeToStr( binSize ), formatDecoding( decoding).lower().replace('and',','), str(isUniform).lower(), combineBins, maxIter )
	...
	if decoding != 'N':
		...
		trInfo = info
		transData = runBaumWelchTransitions( dfClass, transitionMatrix, maxIter, ignoreAr )
		transGroup = transData.groupby( level=0 )
		...
		dfOutput = runDecoding( dfSampleGroup, numProc, transGroup, decoding )
...
def runBaumWelchTransitions( df, initTrans, iter, ignoreAr ):
	# shape df for emissions
	dfP = pd.melt( df, id_vars=['sample','bin','prediction'] )
	dfG = dfP.groupby( 'sample' )
	dfE = dfG.apply( lambda x:(x.pivot(index='variable',columns='bin',values='value')) )
	dfG = dfE.groupby( level = 0 )
	outTrans = dfG.apply( bwTransitions, i=initTrans, x=iter, ia=ignoreAr )
	return outTrans

def bwTransitions( data, i, x, ia ):
	# create object
	bw = TransitionBaumWelch( data, transMat=i, iter=x, ignoreAr=ia )
	return bw.run()

def runDecoding( dfg, numProc, transGroup, decodeType ):
	# get transMat
	mapfunc = partial( decodeSample, trans=transGroup, d=decodeType )
	with multiprocessing.Pool(processes=numProc) as p:
		res = p.map( mapfunc, [group for group in dfg] )
	return pd.concat(res)
...
def decodeSample( group, trans=None, d='V' ):
	name, df = group
	transMat = trans.get_group( name )
	res = decode( df, transMat, d )
	res['sample'] = name
	return res
...
def printHelp():
	...
	print( '-n=max_iter\tmaximum iterations to improve transition matrix [default {:d}]'.format(MAXITER) )
