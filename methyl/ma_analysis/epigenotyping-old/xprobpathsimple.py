...
class ProbPathSimple:
	### This implements the Viterbi algorithm that finds the sequnce with
	### maximum likehood
	
	def __init__(self, df, transitions ):
		self.transMat = transitions #.tolist()
		self.data = df
	...
	def _computeScore( self, i, j, prob ):
		...
		for k in range(3):
			...
			#scores[k] += ( 0 if i == 0 else self.probMat[i-1][k]) + math.log(self.transMat[k][j])
		#print( scores )
		maxS = max( scores )
	...
	def _printTraceMat( self ):
		...
		for i in range(1,20):
			tmpStr = '{:<4d}|'.format( i )
			transAr = [ ' {:>6s}'.format(lAr2[self.traceMat[i][j]]) for j in range(3) ]
			outStr += tmpStr + ''.join(transAr)+ '|\n'
		outStr += ' '*5 + '-'*(3*7)
		print( outStr )
