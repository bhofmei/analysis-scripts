...
# Usage: python epigenotyping_pe_combbin.py [-u] [-c=bin_comb_thresh] [-d=decoding_type] [-p=num_proc] [-o=out_id] [-m=mother_sample] [-f=father_sample] [-b=bin_size] <input_file>
...
def processInputs( inFileStr, numProc, binSize, outID, parentLabelAr, decoding, isUniform, combineBins ):
	...
	info = '#from_script: epigenotyping_pe_combbin_smpt.py; in_file:{:s}; bin_size:{:s}; decoding:{:s}; uni_class_prob:{:s}; combine_bins_threshold:{:d}'.format( os.path.basename( inFileStr ), bth_util.binSizeToStr( binSize ), formatDecoding( decoding).lower().replace('and',','), str(isUniform).lower(), combineBins )
...
def runDecoding( dfg, numProc, decodeType, outF ):
	mapfunc = partial( decodeSample, d=decodeType, f=outF )
	with multiprocessing.Pool(processes=numProc) as p:
		res = p.map( mapfunc, [group for group in dfg] )
	return pd.concat(res)

def decodeSample( group, d='V', f='out.txt' ):
	name, df = group
	# get this groups transitions
	labels = ['mother', 'MPV', 'father']
	trans = newTransitionMatrix( df['prediction'].values, labels )
	with open( f, 'a' ) as ff:
		ff.write(name + '\n')
	newTransTable = pd.DataFrame(trans, labels, labels)
	newTransTable.to_csv( f, sep='\t', mode='a' )
	res = decode( df, trans, d )
	res['sample'] = name
	return res

def newTransitionMatrix( pred, labels ):
		# initialize array
		#print(self.size)
		states= len(labels)
		size = len(pred)
		countAr = np.ones( (states,states) )
		for i in range(size-1):
			l= pred[i:i+2]
			j,k = [ labels.index( x ) for x in (l) ]
			#print('*',j,k)
			countAr[j,k] += 1
