...
class DecodePath:
	...
	'''
	def __init__( self, df, transMat, cent, scaleFactor=1 ):
		# n bins, m states
...
class DecodeViterbi( DecodePath ):
	...
	def _initializeV( self ):
		...
		# adjust transitions based on scale factor
		self.log_transitions = self.log_transitions - math.log(self.scaleFactor)
...
class DecodeForwardBackward( DecodePath ):
	...
	def _initializeF( self ):
		...
		# adjust transitions
		self.transitions = self.transitions / self.scaleFactor
