...
def processInputs(posFileStr, gbmFileStr, gffFileStr, isCDS, isOppo):
	info = '#from_script: filter_pos_gene_gbm.py; pos_file: {:s}; gbm_file: {:s}; gff_file: {:s}; use_cds: {:s}; is_opposite: {:s}'.format( os.path.basename( posFileStr ), ('None' if gbmFileStr == None else os.path.basename( gbmFileStr ) ), os.path.basename( gffFileStr ), str(isCDS), str(isOppo) )
	print( 'Position file:', os.path.basename( posFileStr ))
	print( 'gBM file:', ('None' if gbmFileStr == None else os.path.basename( gbmFileStr )))
	...
	genetype, cdstype = getOutputType( gbmFileStr, isCDS, isOppo )
...
def getOutputType( gbmFileStr, isCDS, isOppo ):
	genetype = ''
	cdstype = 'CDS' if isCDS else 'gene'
	if gbmFileStr == None:
		genetype = 'nongenes' if isOppo else 'genes'
	else:
		genetype = 'ngbm' if isOppo else 'gbm'
...
def readGFF( gffFileStr, gbmAr, isCDS ):
	...
	for line in gffFile:
		...
		if lineAr[2] == "CDS" and isCDS:
			...
			#print( '-', name )
			if len(gbmAr) == 0 or name in gbmAr:
...
def parseInputs( argv ):
	...
	if gbmFileStr.lower() in ['none', 'na']:
		gbmFileStr = None
...
if __name__ == "__main__":
	if len(sys.argv) < 4:
		print ("Usage: python filter_pos_gene_gbm.py [-cds] [-v] <pos_file> <gbm_file> <gff_file>")
	else:
